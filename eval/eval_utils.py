...
def get_test_dataset(dataset_name, tokenizer, seqlen=2048):
    ...
    tokenized_text = [
        tokenizer(item, add_special_tokens=False)["input_ids"]
        + [tokenizer.eos_token_id]
        for item in testdata
    ]

    data, doc = [], [tokenizer.bos_token_id] if tokenizer.add_bos_token else []
    for sen in tokenized_text:
        if len(sen) > seqlen:
            continue
        if len(doc) + len(sen) > seqlen:
            data.append(doc)
            doc = [tokenizer.bos_token_id] if tokenizer.add_bos_token else []
        doc.extend(sen)
...
class LMEvalAdaptor(BaseLM):
    def __init__(
        self, model_name, model, tokenizer, batch_size=1, max_length=-1, device="cuda:0"
    ):
        super().__init__()
